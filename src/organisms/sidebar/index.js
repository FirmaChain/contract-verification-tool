...
import { SidebarContainer, InputWrap, Label, QueryButton, TopContent, GeneralButton, BottomContent, InputWrapRight, ResultContainer, RawLog } from "./styles";
import {
...
const SideBar = () => {
    ...
    const addContractLog = async () => {
        try {
            const result = await verifyAddContractLog(wallet, file.file, textValue);
            handleQueryResult(result);
        } catch (error) {
            throw error;
        }
